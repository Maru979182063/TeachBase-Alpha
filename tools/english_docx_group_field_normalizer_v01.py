...
def field_order_for_kind(kind: str) -> list[str]:
    if kind in WRITING_KINDS:
        return WRITING_FIELD_ORDER
    return FIELD_ORDER
...
def block_markdown(block_id: str, blocks_by_id: dict[str, dict[str, Any]]) -> str:
    block = blocks_by_id.get(block_id) or {}
    return str(block.get("display_markdown") or "").strip()


def header_residue(text: str) -> str:
    residue = str(text or "").strip()
    for token in SAMPLE_ANSWER_HEADER_TOKENS:
...
def is_sample_answer_header_only(block_ids: list[str], blocks_by_id: dict[str, dict[str, Any]]) -> bool:
    text = "\n".join(block_markdown(block_id, blocks_by_id) for block_id in block_ids).strip()
    return bool(text) and not header_residue(text)


def first_teaching_note_index(block_ids: list[str], blocks_by_id: dict[str, dict[str, Any]]) -> int:
    for index, block_id in enumerate(block_ids):
        text = block_markdown(block_id, blocks_by_id)
        if any(marker in text for marker in TEACHING_NOTE_MARKERS):
            return index
    return -1


def normalize_writing_field_contract(
    field_block_ids: dict[str, list[str]],
    field_confidence: dict[str, str],
    blocks_by_id: dict[str, dict[str, Any]],
    warnings: list[str],
) -> None:
    answer_ids = field_block_ids.get("answer") or []
    sample_ids = field_block_ids.get("sample_answer") or []
    if answer_ids and sample_ids and is_sample_answer_header_only(answer_ids, blocks_by_id):
        field_block_ids["sample_answer"] = sorted(answer_ids + sample_ids, key=source_order)
        field_block_ids["answer"] = []
        field_confidence["sample_answer"] = field_confidence.get("sample_answer") or field_confidence.get("answer") or "unknown"
        field_confidence.pop("answer", None)
        warnings.append("writing_answer_header_folded_into_sample_answer")
    elif answer_ids:
        answer_text = materialize_markdown(answer_ids, blocks_by_id)
        if len(answer_text) > 80 or "\n" in answer_text:
            field_block_ids["sample_answer"] = sorted(sample_ids + answer_ids, key=source_order)
            field_block_ids["answer"] = []
            field_confidence["sample_answer"] = field_confidence.get("sample_answer") or field_confidence.get("answer") or "unknown"
            field_confidence.pop("answer", None)
            warnings.append("writing_long_answer_reclassified_as_sample_answer")

    explanation_ids = field_block_ids.get("explanation") or []
    teaching_index = first_teaching_note_index(explanation_ids, blocks_by_id)
    if teaching_index >= 0:
        kept = explanation_ids[:teaching_index]
        moved = explanation_ids[teaching_index:]
        field_block_ids["explanation"] = kept
        field_block_ids["teaching_note"] = sorted((field_block_ids.get("teaching_note") or []) + moved, key=source_order)
        field_confidence["teaching_note"] = field_confidence.get("teaching_note") or field_confidence.get("explanation") or "unknown"
        warnings.append("writing_teaching_notes_split_from_explanation")
...
def materialize_record(normalized: dict[str, Any], group: dict[str, Any], blocks_by_id: dict[str, dict[str, Any]]) -> dict[str, Any]:
    normalized_kind = str(normalized.get("normalized_kind") or "")
    ...
    if normalized_kind in WRITING_KINDS:
        normalize_writing_field_contract(field_block_ids, field_confidence, blocks_by_id, warnings)
    ordered_fields = field_order_for_kind(normalized_kind)
    fields = {field: materialize_markdown(field_block_ids.get(field, []), blocks_by_id) for field in ordered_fields}
    field_asset_refs = {
        field: field_assets(field_block_ids.get(field, []), blocks_by_id)
        for field in ordered_fields
        if field_assets(field_block_ids.get(field, []), blocks_by_id)
    }
    return {
        ...
        "normalization_block_ids": group.get("normalization_block_ids") or group.get("source_block_ids") or [],
        "excluded_block_ids": group.get("excluded_block_ids") or [],
        "protocol_projection": group.get("protocol_projection") or {},
        "field_block_ids": {field: field_block_ids.get(field, []) for field in ordered_fields if field_block_ids.get(field)},
...
def normalization_block_ids(group: dict[str, Any]) -> list[str]:
    source_ids = [str(value) for value in group.get("source_block_ids") or []]
    projection = group.get("protocol_projection") or {}
    discarded = {str(value) for value in projection.get("discarded_block_ids") or []}
    if not projection:
        return source_ids
    return [bid for bid in source_ids if bid not in discarded]


def excluded_block_ids(group: dict[str, Any]) -> list[str]:
    source_ids = {str(value) for value in group.get("source_block_ids") or []}
    projection = group.get("protocol_projection") or {}
    discarded = [str(value) for value in projection.get("discarded_block_ids") or []]
    return [bid for bid in discarded if bid in source_ids]
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for group in selected_groups:
        ids = normalization_block_ids(group)
        prepared_group = {
            **group,
            "normalization_block_ids": ids,
            "excluded_block_ids": excluded_block_ids(group),
        }
