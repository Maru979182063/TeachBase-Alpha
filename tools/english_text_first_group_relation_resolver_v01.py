...
def compact_input(
    ...
    if base_graph:
        ...
        payload["locked_nodes"] = [
            node
            for node in base_graph.get("nodes", [])
            if isinstance(node, dict) and node.get("document_group_id") in group_ids
        ]
        payload["existing_relations"] = [
            relation
            for relation in base_graph.get("relations", [])
            if isinstance(relation, dict)
            and relation.get("subject_group_id") in group_ids
            and relation.get("object_group_id") in group_ids
        ]
    return payload
...
def validate_graph(graph: dict[str, Any], *, doc_id: str, group_ids: set[str], block_refs: set[str], prompt_version: str) -> dict[str, Any]:
    ...
    necessities = {"required", "supporting", "optional", "unknown"}
    consumption_policies = {
        "must_attach",
        "may_attach",
        "do_not_attach_to_question_body",
        "projection_ownership_only",
        "review",
    }
    for index, relation in enumerate(graph.get("relations", [])):
        ...
        if "context_role" in relation and relation.get("context_role") not in context_roles:
            errors.append({"path": f"$.relations[{index}].context_role", "message": "invalid context_role"})
        if "necessity" in relation and relation.get("necessity") not in necessities:
            errors.append({"path": f"$.relations[{index}].necessity", "message": "invalid necessity"})
        if "consumption_policy" in relation and relation.get("consumption_policy") not in consumption_policies:
            errors.append({"path": f"$.relations[{index}].consumption_policy", "message": "invalid consumption_policy"})
...
def force_base_nodes(graph: dict[str, Any], base_graph: dict[str, Any], group_ids: set[str]) -> dict[str, Any]:
    """Keep accepted node projection decisions while allowing relation annotations to change."""
    base_nodes = [
        node
        for node in base_graph.get("nodes", [])
        if isinstance(node, dict) and node.get("document_group_id") in group_ids
...
def render_review(summary: dict[str, Any], graph: dict[str, Any]) -> str:
    ...
    for rel in graph.get("relations", []):
        rel_rows.append(
            ...
            f"<td>{rel.get('context_role','')}</td>"
            f"<td>{rel.get('necessity','')}</td>"
            f"<td>{rel.get('consumption_policy','')}</td>"
            f"<td>{rel.get('confidence','')}</td>"
            f"<td>{rel.get('independence_test','')}</td>"
...
<h2>Relations</h2>
<table><thead><tr><th>subject</th><th>predicate</th><th>object</th><th>context_role（关系用途）</th><th>necessity（必要性）</th><th>consumption_policy（消费策略）</th><th>confidence</th><th>independence_test（独立性测试）</th><th>reason</th></tr></thead><tbody>{''.join(rel_rows)}</tbody></table>
...
def call_relation_model_for_groups(
    *,
    ...
    prompt_version: str,
    base_graph: dict[str, Any] | None = None,
    preserve_base_nodes: bool = False,
) -> dict[str, Any]:
    input_payload = compact_input(doc_id, groups, normalized, block_index, base_graph)
    ...
    if graph is not None and base_graph and preserve_base_nodes:
        graph = force_base_nodes(graph, base_graph, {group["document_group_id"] for group in groups})
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    base_graph = read_json(workspace_path(args.base_graph_json)) if args.base_graph_json else None
    ...
    if use_chunking:
        ...
        for index, chunk_groups in enumerate(chunks, start=1):
            ...
            result = call_relation_model_for_groups(
                ...
                preserve_base_nodes=args.preserve_base_nodes,
            )
            chunk_record = {
                "chunk_id": label,
        ...
        if base_graph and args.preserve_base_nodes:
            graph = force_base_nodes(graph, base_graph, {group["document_group_id"] for group in groups})
        group_ids = {group["document_group_id"] for group in groups}
        block_refs = {ref for group in groups for ref in all_group_refs(group)}
...
def main() -> None:
    ...
    parser.add_argument("--base-graph-json", default="")
    parser.add_argument("--preserve-base-nodes", action="store_true")
