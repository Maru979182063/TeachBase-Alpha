...
def load_field_role_resolutions(paths: list[Path]) -> dict[str, dict[str, Any]]:
    ...
    for path in paths:
        ...
        for item in payload.get("items") or []:
            ...
            group_id = str(resolution.get("source_group_id") or (item.get("draft") or {}).get("source_group_id") or "")
            if not group_id:
                continue
            copy = dict(resolution)
            copy["_source_path"] = rel_workspace(path)
            resolutions[group_id] = copy
    return resolutions


def normalize_field_role_resolution(role_resolution: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        "available": True,
        "source": str(role_resolution.get("_source_path") or ""),
        "overall_assessment": role_resolution.get("overall_assessment") or {},
        "field_roles": list(role_resolution.get("field_roles") or []),
        "warnings": list(role_resolution.get("warnings") or []),
    }
...
def build_packet(draft: dict[str, Any], role_resolution: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "field_role_resolution": normalize_field_role_resolution(role_resolution),
...
def validate_packet(packet: dict[str, Any]) -> dict[str, Any]:
    ...
    surfaces = packet.get("source_surfaces")
    if not isinstance(surfaces, dict):
        errors.append({"path": "$.source_surfaces", "message": "missing source_surfaces"})
    else:
        for field_name in SOURCE_SURFACE_FIELDS:
            field = surfaces.get(field_name)
            if not isinstance(field, dict):
                errors.append({"path": f"$.source_surfaces.{field_name}", "message": "missing field"})
                continue
            for key in ("text", "refs", "missing_refs"):
                if key not in field:
                    errors.append({"path": f"$.source_surfaces.{field_name}.{key}", "message": "missing key"})
...
def render_packet_card(packet: dict[str, Any]) -> str:
    ...
    role_rows = []
    for role in (packet.get("field_role_resolution") or {}).get("field_roles") or []:
        role_rows.append(
            "<tr>"
    ...
    roles_html = (
        "<table><thead><tr><th>field</th><th>role</th><th>display</th><th>layout</th><th>reason</th></tr></thead>"
        f"<tbody>{''.join(role_rows)}</tbody></table>"
        if role_rows
        else "<p class='muted'>no Node4b field-role sidecar</p>"
    )
    return f"""
<article class="card">
  ...
  <details open><summary>field_role_resolution（Node4b 字段角色/展示策略）</summary>{roles_html}</details>
...
td,th{{border:1px solid #d8dce2;padding:6px;vertical-align:top}}
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    role_resolutions = load_field_role_resolutions([workspace_path(path) for path in args.field_role_resolutions_json])

    packet_candidates = []
    packet_records = []
    for draft in drafts:
        packet = build_packet(draft, role_resolutions.get(str(draft.get("source_group_id") or "")))
    ...
    payload = {
        ...
        "field_role_resolution_sources": [rel_workspace(workspace_path(path)) for path in args.field_role_resolutions_json],
...
def main() -> None:
    ...
    parser.add_argument("--field-role-resolutions-json", nargs="*", default=[])
