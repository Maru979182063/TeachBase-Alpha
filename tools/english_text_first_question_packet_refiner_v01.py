...
SOURCE_SURFACE_FIELD_NAMES = ["visual", "writing_surface"]
...
def empty_source_surfaces() -> dict[str, dict[str, Any]]:
    return {
        name: {"text": "", "refs": [], "missing_refs": []}
        for name in SOURCE_SURFACE_FIELD_NAMES
    }


def normalize_packet_field(value: Any) -> dict[str, Any]:
    if not isinstance(value, dict):
        value = {}
    return {
        "text": str(value.get("text") or ""),
        "refs": [str(ref) for ref in value.get("refs") or [] if str(ref).strip()],
        "missing_refs": [str(ref) for ref in value.get("missing_refs") or [] if str(ref).strip()],
    }


def packet_source_surfaces(packet: dict[str, Any]) -> dict[str, dict[str, Any]]:
    surfaces = packet.get("source_surfaces") if isinstance(packet.get("source_surfaces"), dict) else {}
    content = packet.get("content") if isinstance(packet.get("content"), dict) else {}
    result = empty_source_surfaces()
    for name in SOURCE_SURFACE_FIELD_NAMES:
        field = surfaces.get(name)
        if not isinstance(field, dict):
            field = content.get(name)
        result[name] = normalize_packet_field(field)
    return result


def normalize_source_surfaces(value: Any, packet: dict[str, Any]) -> dict[str, dict[str, Any]]:
    # source_surfaces is evidence passthrough, not model-authored content.
    return packet_source_surfaces(packet)
...
def parse_option_text(options_text: str) -> list[dict[str, str]]:
    text = str(options_text or "").replace("\xa0", " ").strip()
    if not text:
        return []
    ...
    for index, char in enumerate(text):
        ...
        if before.isalnum() or before in {"_", "/"}:
            continue
        markers.append((index, char, index + 2))
    if not markers:
        return []
    ...
    for start_idx, marker in enumerate(markers):
        ...
        seq = [marker]
        expected = 1
        for next_marker in markers[start_idx + 1 :]:
            if expected >= len(letters):
                break
            if next_marker[1] == letters[expected]:
                seq.append(next_marker)
                expected += 1
                if len(seq) >= 4:
                    break
        if len(seq) > len(best):
    ...
    for idx, (start, label, text_start) in enumerate(best):
        if idx + 1 < len(best):
            end = best[idx + 1][0]
        else:
            next_markers = [marker_start for marker_start, _marker_label, _text_start in markers if marker_start > start]
            end = min(next_markers) if next_markers else len(text)
        value = " ".join(text[text_start:end].strip().split())
        if value:
            options.append({"label": label, "text": value})
    return options


def clean_preserved_stem(stem: str, translation: str) -> str:
    paragraphs = [part.strip() for part in str(stem or "").split("\n\n") if part.strip()]
    if len(paragraphs) < 2:
        return str(stem or "")
    first = paragraphs[0]
    second = paragraphs[1]
    if first and first in second and has_cjk(second) and first in str(translation or ""):
        return "\n\n".join([first] + paragraphs[2:])
    return str(stem or "")
...
def source_text_corpus(packet: dict[str, Any]) -> str:
    ...
    for value in (packet.get("fields") or {}).values() if isinstance(packet.get("fields"), dict) else []:
        if isinstance(value, dict):
            ...
            parts.append(str(value or ""))
    for value in packet_source_surfaces(packet).values():
        parts.append(str(value.get("text") or ""))
    return "\n".join(parts)
...
def refined_user_text(refined: dict[str, Any]) -> str:
    ...
    for key in empty_standard_question():
        ...
        if isinstance(value, list):
            ...
            parts.append(str(value or ""))
    for value in (refined.get("source_surfaces") or {}).values() if isinstance(refined.get("source_surfaces"), dict) else []:
        if isinstance(value, dict):
            parts.append(str(value.get("text") or ""))
    return "\n".join(parts)


def cloze_core_blank_preserved(packet: dict[str, Any], refined: dict[str, Any]) -> bool:
    if str(packet.get("packet_family") or "").strip().lower() != "cloze":
        return False
    source_core = "\n".join(
        text
        for text in [field_text(packet, "stem"), field_text(packet, "passage")]
        if text.strip()
    )
    question = refined.get("standard_question") or {}
    output_core = "\n".join(
        str(question.get(key) or "")
        for key in ["passage", "stem"]
    )
    output_core = "\n".join([output_core, str(refined.get("final_markdown") or "")])
    return blank_run_count(source_core) > 0 and blank_run_count(output_core) > 0
...
def deterministic_preserve(packet: dict[str, Any], prompt_version: str, status: str, reason: str) -> dict[str, Any]:
    ...
    standard["options"] = parse_option_text(str((content.get("options") or {}).get("text") or ""))
    standard["stem"] = clean_preserved_stem(standard["stem"], standard["translation"])
    ...
    refined = {
        ...
        "source_surfaces": packet_source_surfaces(packet),
...
def repair_refined_shape(refined: dict[str, Any], packet: dict[str, Any], prompt_version: str) -> dict[str, Any]:
    ...
    previous_surfaces = repaired.get("source_surfaces")
    repaired["source_surfaces"] = normalize_source_surfaces(previous_surfaces, packet)
            ...
            normalized_warnings.append(
                {
                    ...
                    **{key: value for key, value in item.items() if key not in {"code", "message", "refs"}},
                }
            )
        else:
            normalized_warnings.append(
                {
                    "code": "warning",
                    "message": str(item),
                    "refs": [str(packet.get("packet_id") or "")],
                }
            )
    repaired["warnings"] = normalized_warnings
    if previous_surfaces != repaired["source_surfaces"]:
        repaired["normalization_actions"].append("source_surfaces_lossless_passthrough")
...
def validate_refined(refined: dict[str, Any], packet: dict[str, Any], prompt_version: str) -> dict[str, Any]:
    ...
    warnings_value = refined.get("warnings")
    if not isinstance(warnings_value, list):
        errors.append({"path": "$.warnings", "message": "warnings must be array"})
    else:
        for index, warning in enumerate(warnings_value):
            if not isinstance(warning, dict):
                errors.append({"path": f"$.warnings[{index}]", "message": "warning must be object"})
            elif "code" not in warning or "message" not in warning:
                errors.append({"path": f"$.warnings[{index}]", "message": "warning requires code and message"})
    if (
        packet.get("projection_status") == "PRESERVED_NON_DIRECT"
    ...
    if refined.get("refine_status") not in {"PRESERVED_NON_DIRECT", "REFINE_FAILED"}:
        ...
        if (
            source_blank_runs
            and output_blank_runs < source_blank_runs
            and not cloze_core_blank_preserved(packet, refined)
        ):
            errors.append(
                {
                    "path": "$.standard_question",
    ...
    expected_surfaces = packet_source_surfaces(packet)
    output_surfaces = refined.get("source_surfaces")
    if not isinstance(output_surfaces, dict):
        errors.append({"path": "$.source_surfaces", "message": "missing source surface passthrough"})
    else:
        for name, expected in expected_surfaces.items():
            output = normalize_packet_field(output_surfaces.get(name))
            if expected != output:
                errors.append(
                    {
                        "path": f"$.source_surfaces.{name}",
                        "message": "source surface evidence must be copied losslessly from input packet",
                    }
                )
...
def reconcile_refine_status(refined: dict[str, Any]) -> dict[str, Any]:
    ...
    if refined.get("refine_status") == "REFINED_READY":
        ...
        warnings.append(
            {
                "code": "local_status_gate_downgraded_ready",
                "message": "Local status gate downgraded READY because status_breakdown is not fully clean/ready.",
                "refs": [str(refined.get("source_packet_id") or "")],
            }
        )
    return refined
...
def refine_one(
    *,
    ...
    input_payload = {
        ...
        "node5a_candidate_audit": candidate_audit or {},
            "refiner_policy": {
            "one_packet_only": True,
            "exact_source_only": True,
            "do_not_invent_missing_fields": True,
            "honor_node5a_constraints": bool(candidate_audit),
    ...
    write_json(packet_dir / "input_packet_candidate.json", packet)
    if candidate_audit:
        write_json(packet_dir / "node5a_candidate_audit.json", candidate_audit)
...
def load_candidate_audits(path_text: str) -> dict[str, dict[str, Any]]:
    if not path_text:
        return {}
    payload = read_json(workspace_path(path_text))
    result: dict[str, dict[str, Any]] = {}
    for item in payload.get("items") or []:
        audit = item.get("audit") or {}
        for key in [item.get("packet_id"), item.get("source_group_id"), audit.get("packet_id"), audit.get("source_group_id")]:
            if key:
                result[str(key)] = audit
    return result
...
def render_review(refined_packets: list[dict[str, Any]], summary: dict[str, Any]) -> str:
    ...
    for packet in refined_packets:
        ...
        source_surfaces = html.escape(json.dumps(packet.get("source_surfaces") or {}, ensure_ascii=False, indent=2))
...
<article class="card">
  ...
  {surface_summary}
  <details open><summary>source_surfaces (source visual/writing surface evidence)</summary><pre>{source_surfaces}</pre></details>
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    audit_by_id = load_candidate_audits(args.candidate_audits_json)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_by_packet = {
            executor.submit(
                ...
                api_key=api_key,
                out_dir=out_root,
                candidate_audit=audit_by_id.get(str(packet.get("packet_id") or "")) or audit_by_id.get(str(packet.get("source_group_id") or "")),
            ): packet
    ...
    summary = {
        ...
        "candidate_audits_json": rel_workspace(workspace_path(args.candidate_audits_json)) if args.candidate_audits_json else "",
...
def main() -> None:
    ...
    parser.add_argument("--candidate-audits-json", default="")
