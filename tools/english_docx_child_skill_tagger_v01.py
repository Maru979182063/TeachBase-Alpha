...
CLOZE_ALLOWED_PRIMARY_LABELS_ZH = {
    "名词｜相关内容",
    "名词｜名动搭配",
    "名词｜形名修饰",
    "名词｜上下文语境",
...
def render_user_prompt(
    ...
    *,
    ...
    children = [
        child_for_model(child, max_chars)
        for child in group.get("children") or []
        if str(child.get("item_kind") or "") == "cloze_choice"
    ]
...
def validate_tags(parsed: dict[str, Any] | None, *, doc_key: str, group: dict[str, Any]) -> tuple[bool, list[str], dict[str, Any]]:
    ...
    child_kind_by_id = {
        str(child.get("item_id") or ""): str(child.get("item_kind") or "")
        for child in group.get("children") or []
    }
    ...
    for index, item in enumerate(items):
        ...
        if not isinstance(secondary, list):
            secondary = []
        primary_label_zh = str(item.get("primary_label_zh") or "unknown")
        item_kind = child_kind_by_id.get(item_id)
        if item_kind == "cloze_choice" and primary_label_zh not in CLOZE_ALLOWED_PRIMARY_LABELS_ZH:
            issues.append(f"invalid_cloze_primary_label:{item_id}:{primary_label_zh}")
        relevant_context = str(item.get("relevant_context") or "").strip()
        if item_kind == "cloze_choice" and item_no:
            relevant_context = relevant_context.replace(f"[[BLANK_{item_no}]]", f"[[CURRENT_BLANK_{item_no}]]")
        if item_kind == "cloze_choice" and not relevant_context:
            issues.append(f"missing_relevant_context:{item_id}")
        if item_kind == "cloze_choice" and ("..." in relevant_context or "…" in relevant_context):
            issues.append(f"ellipsis_in_relevant_context:{item_id}")
        if item_kind == "cloze_choice" and "[[CURRENT_BLANK_" not in relevant_context:
            issues.append(f"missing_current_blank_in_relevant_context:{item_id}")
        normalized_items.append(
            {
                "item_id": item_id,
                "item_no": item_no,
                "relevant_context": relevant_context,
                "primary_label_zh": primary_label_zh,
...
def merge_tags(group: dict[str, Any], tag_result: dict[str, Any]) -> dict[str, Any]:
    ...
    for child in group.get("children") or []:
        updated = dict(child)
        tag = tags.get(child.get("item_id")) or {
            "primary_label_zh": "未知",
        ...
        if "question_full" not in updated:
            updated["question_full"] = updated.get("question") or ""
        if tag.get("relevant_context"):
            updated["relevant_context"] = tag.get("relevant_context") or ""
        updated["skill_tags"] = {
            key: value
            for key, value in tag.items()
            if key not in {"relevant_context", "item_id", "item_no"}
        }
...
def process_group(
    *,
    ...
    if not any(str(child.get("item_kind") or "") == "cloze_choice" for child in group.get("children") or []):
        return {
            "group_id": group_id,
            "status": "skipped_non_cloze",
            "tagged_group": group,
            "issues": [],
            "usage": {},
        }
...
def compact_review_context(child: dict[str, Any]) -> str:
    explicit = str(child.get("relevant_context") or "").strip()
    question = str(child.get("question") or "").strip()
    if explicit and explicit != question:
        return explicit
    if str(child.get("item_kind") or "") != "cloze_choice":
        return explicit or question
    marker = f"[[CURRENT_BLANK_{child.get('source_item_no') or child.get('item_no') or ''}]]"
    if marker not in question:
        return explicit or question
    start = question.find(marker)
    left = max(question.rfind(". ", 0, start), question.rfind("? ", 0, start), question.rfind("! ", 0, start))
    current_start = 0 if left < 0 else left + 2
    right_candidates = [pos for pos in [question.find(". ", start), question.find("? ", start), question.find("! ", start)] if pos >= 0]
    current_end = (min(right_candidates) + 1) if right_candidates else len(question)
    current = question[current_start:current_end].strip()
    first_word = current.split(" ", 1)[0].strip("“\"'").lower()
    needs_previous = first_word in {"it", "this", "that", "these", "those", "they", "he", "she"}
    if needs_previous and current_start > 0:
        prev_left = max(question.rfind(". ", 0, max(0, current_start - 2)), question.rfind("? ", 0, max(0, current_start - 2)), question.rfind("! ", 0, max(0, current_start - 2)))
        prev_start = 0 if prev_left < 0 else prev_left + 2
        return (question[prev_start:current_start].strip() + " " + current).strip()
...
def render_index(groups: list[dict[str, Any]], out_path: Path) -> None:
    ...
    for group in groups:
        ...
        for field in parent_field_order:
            value = parent.get(field)
            if not str(value or "").strip():
                continue
            parent_bits.append(
                '<section class="parent-field">'
                f'<h5>{html.escape(parent_field_labels.get(field, field))}</h5>'
        ...
        for child in group.get("children") or []:
            ...
            child_bits.append(
                ...
                f'<h5>相关原文</h5>{render_text(compact_review_context(child))}'
                f'<h5>选项</h5>{render_text(child.get("options") or "")}'
        ...
        sections.append(
            ...
            f'<div class="parent"><h3>父级内容</h3>{"".join(parent_bits)}</div>'
            '<h3>子题</h3>'
            f'{"".join(child_bits)}'
            "</section>"
        )
...
h3{margin:14px 0 8px;color:#334155;font:700 16px/1.35 "Microsoft YaHei",sans-serif}
.parent{padding:12px 14px;margin:8px 0 16px;background:#f8fafc;border:1px solid #d8dee5;border-radius:6px}
.parent-field{padding:6px 0;border-top:1px solid #e5eaf0}.parent-field:first-of-type{border-top:0}
