...
def classify_bad_case(case: dict[str, Any], prediction: dict[str, Any]) -> list[str]:
    ...
    if (
        (
            _pred_role(prediction) != case.get("expected_semantic_role")
            or _pred_route(prediction) != case.get("expected_route_candidate")
        )
...
def compute_metrics(cases: list[dict[str, Any]], predictions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for case in verified:
        ...
        if not all_primary_ok:
            ...
            categories = classify_bad_case(case, pred)
            bad_cases.append({**wrong_entry, "categories": categories})
