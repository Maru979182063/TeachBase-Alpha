...
PROMPT_VERSION = "english_question_render_normalizer_v1.4_structured_components_20260724"
...
INLINE_MARK_TYPES = {"underline"}
SURFACE_BLANK_CHARS = {"_", "＿", "—", "－", "–", "─", "━"}
MARKDOWN_RENDERER = MarkdownIt("default", {"html": True})
...
def page_image_paths(packet: dict[str, Any]) -> list[Path]:
    ...
    def add_path(path: Path, page: int | None = None) -> None:
        if not path.exists():
            return
        resolved_page = page
        if resolved_page is None:
            resolved_page = page_number_from_ref(path.stem.replace("page_", "_p"))
    ...
    for page in source_pages_from_packet(packet):
        for path in candidate_page_image_paths(str(packet.get("doc_id") or ""), page):
            add_path(path, page)
...
def candidate_page_image_paths(doc_id: str, page: int) -> list[Path]:
    page_name = f"page_{page:03d}.png"
    doc = str(doc_id or "")
    family_dir = ""
    if doc.startswith("reading_"):
        family_dir = "reading"
    elif doc.startswith("writing_"):
        family_dir = "writing"
    elif doc.startswith("grammar_"):
        family_dir = "grammar"
    candidates = []
    if family_dir:
        candidates.append(
            workspace_path(
                f"outputs/english_text_first_pipeline_v02_spec_20260715/controlled_runs/"
                f"source_page_images_3docs_20260724_full/{family_dir}/{page_name}"
            )
        )
        candidates.append(
            workspace_path(
                f"outputs/english_text_first_pipeline_v02_spec_20260715/controlled_runs/"
                f"node5b6b_pdf_visual_audit_20260724/page_images/page_images/{family_dir}/{page_name}"
            )
        )
    if family_dir == "grammar":
        candidates.append(
            workspace_path(
                f"outputs/english_text_first_pipeline_v01_20260714/grammar_clauses_p001_p008_window4_live_v2/"
                f"evidence/page_images/{page_name}"
            )
        )
    return [path for path in candidates if path.exists()]


def source_pages_from_packet(packet: dict[str, Any]) -> list[int]:
    pages: list[int] = []
    ...
    for key in ["visual_refs", "writing_surface_refs"]:
        values.extend(asset_refs.get(key) or [])
    for value in values:
        page = page_number_from_ref(str(value))
        if page is not None and page not in pages:
            pages.append(page)
    return pages


def surface_assets_for_packet(packet: dict[str, Any]) -> list[dict[str, Any]]:
    assets: list[dict[str, Any]] = []
    seen_paths: set[str] = set()
    seen_pages: set[int] = set()
    asset_refs = packet.get("asset_refs") or {}
    requested_refs = list(asset_refs.get("visual_refs") or []) + list(asset_refs.get("writing_surface_refs") or [])
    for item in asset_refs.get("page_image_refs") or []:
        path = workspace_path(item.get("path") or "")
        if path.exists():
                ...
                assets.append(
                    {
                        ...
                        "caption": f"source page {item.get('page') or ''}".strip(),
                    }
                )
                seen_paths.add(key)
                try:
                    seen_pages.add(int(item.get("page")))
                except (TypeError, ValueError):
                    page = page_number_from_ref(path.stem.replace("page_", "_p"))
                    if page is not None:
                        seen_pages.add(page)
    if requested_refs:
        for page in source_pages_from_packet(packet):
            ...
            for path in candidate_page_image_paths(str(packet.get("doc_id") or ""), page):
                ...
                assets.append(
                    {
                        "kind": "source_page_fallback",
                        "path": rel_workspace(path),
                        "caption": f"page {page:03d} fallback for visual/writing surface refs",
                    }
                )
                seen_paths.add(key)
                seen_pages.add(page)
    if requested_refs and not assets:
        assets.append(
            {
                "kind": "missing_surface_asset",
                "path": "",
                "caption": "visual/writing surface refs exist, but no renderable source image or crop file was found",
                "refs": requested_refs,
            }
        )
    return assets
...
def renderable_surface_asset_present(display: dict[str, Any]) -> bool:
    assets = display.get("surface_assets") if isinstance(display.get("surface_assets"), list) else []
    for asset in assets:
        if not isinstance(asset, dict):
            continue
        if asset.get("kind") == "missing_surface_asset":
            continue
        path_text = str(asset.get("path") or "")
        if path_text and workspace_path(path_text).exists():
            return True
    return False
...
def with_display_projection(packet: dict[str, Any], display_projection: dict[str, Any] | None) -> dict[str, Any]:
    if not display_projection:
        return packet
    merged = json.loads(json.dumps(packet, ensure_ascii=False))
    merged["display_projection"] = {
        "display_mode": display_projection.get("display_mode"),
        "primary_material_refs": display_projection.get("primary_material_refs") or [],
        "weak_context_refs": display_projection.get("weak_context_refs") or [],
        "surface_refs": display_projection.get("surface_refs") or [],
        "layout_sections": display_projection.get("layout_sections") or [],
        "binding_decisions": display_projection.get("binding_decisions") or {},
        "review_requirements": display_projection.get("review_requirements") or [],
        "version": display_projection.get("version"),
    }
    return merged
...
def build_input_payload(packet: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "display_projection": packet.get("display_projection") or {},
    ...
    while i < len(lines):
        ...
        if stripped.startswith("|") and i + 1 < len(lines) and is_table_separator(lines[i + 1]):
            ...
            while i < len(lines) and lines[i].strip().startswith("|"):
                ...
                i += 1
            out.extend(clean_markdown_table_lines(table_lines))
            continue
        if stripped.startswith("|") and is_table_separator(stripped):
            out.append(lines[i])
            i += 1
            continue
        if stripped.startswith("|") and not (i + 1 < len(lines) and is_table_separator(lines[i + 1])):
            run: list[str] = []
            while i < len(lines):
                candidate = lines[i].strip()
                if candidate.startswith("|") and not is_table_separator(candidate):
                    run.append(lines[i])
                    i += 1
                    continue
                break
            if len(run) < 2:
                out.extend(run)
                continue
            rows = [[cell.strip() for cell in row.strip().strip("|").split("|")] for row in run]
...
def clean_markdown_table_lines(lines: list[str]) -> list[str]:
    if len(lines) < 2:
        return lines
    cleaned = [lines[0], lines[1]]
    for line in lines[2:]:
        stripped = line.strip()
...
def unsupported_display_lines(record: dict[str, Any], packet: dict[str, Any]) -> list[str]:
    visual_recovered_text = "\n".join(
        str(section.get("recovered_markdown") or "")
        for section in record.get("visual_recovered_sections") or []
        if isinstance(section, dict)
    )
    source_text = "\n".join([render_source_text(packet), visual_recovered_text]).strip()
    return unsupported_lines(source_text=source_text, output_text=display_user_text(record))


def has_image_surface_evidence(packet: dict[str, Any]) -> bool:
    asset_refs = packet.get("asset_refs") or {}
    page_images = asset_refs.get("page_image_refs") or []
    return bool(page_images or asset_refs.get("visual_refs") or asset_refs.get("writing_surface_refs") or page_image_paths(packet))
...
def standard_source_fields(packet: dict[str, Any]) -> dict[str, str]:
    ...
    for node in projection_context.get("resolved_parent_nodes") or []:
        ...
        if text:
            append_unique_text(parent_chunks, text)
    return {
...
def repair_plan_shape(payload: dict[str, Any] | None, packet: dict[str, Any], reason: str = "") -> dict[str, Any]:
    ...
    for mark in payload.get("inline_marks") if isinstance(payload.get("inline_marks"), list) else []:
        if not isinstance(mark, dict):
            continue
        display_area = str(mark.get("display_area") or "")
        source_field = str(mark.get("source_field") or "")
        mark_type = str(mark.get("mark") or "")
        text = str(mark.get("text") or "").strip()
        if display_area not in DISPLAY_TARGET_FIELDS or source_field not in DISPLAY_SOURCE_FIELDS:
            continue
        if mark_type not in INLINE_MARK_TYPES or not text:
            continue
        inline_marks.append(
            {
                "display_area": display_area,
...
def display_projection_layout_sections(packet: dict[str, Any]) -> list[dict[str, Any]]:
    display_projection = packet.get("display_projection") or {}
    sections: list[dict[str, Any]] = []
    for section in display_projection.get("layout_sections") or []:
        if not isinstance(section, dict):
            continue
        display_area = str(section.get("display_area") or "")
        if display_area not in DISPLAY_TARGET_FIELDS:
        ...
        sections.append(
            {
                "section_id": str(section.get("section_id") or f"display_projection_{len(sections) + 1:03d}"),
                "display_area": display_area,
                "source_fields": source_fields,
                "render_as": render_as if render_as in LAYOUT_RENDER_AS else "source_markdown",
                "reason": str(section.get("reason") or "display_projection sidecar"),
            }
        )
    return sections


def apply_display_projection_floor(plan: dict[str, Any], packet: dict[str, Any]) -> dict[str, Any]:
    """Keep deterministic display projection visible even when the model omits it."""
    display_projection = packet.get("display_projection") or {}
    if not display_projection:
        return plan
    projection_sections = display_projection_layout_sections(packet)
    if projection_sections:
        # With a display projection sidecar, field placement is deterministic.
        # The model may still add visual_recovered_sections and inline_marks,
        # but it should not decide where existing refined fields are displayed.
        plan["layout_sections"] = projection_sections
    bindings = plan.setdefault("binding_decisions", {})
    projection_bindings = display_projection.get("binding_decisions") if isinstance(display_projection.get("binding_decisions"), dict) else {}
    for key, value in projection_bindings.items():
        if key not in {"parent_context", "stimulus", "visual_surface", "writing_surface"} or not isinstance(value, dict):
            continue
        current = bindings.get(key)
        current_required = bool(current.get("required")) if isinstance(current, dict) else False
        if not isinstance(current, dict) or (value.get("required") and not current_required):
            bindings[key] = value
    for item in display_projection.get("review_requirements") or []:
        if isinstance(item, dict):
            plan.setdefault("review_requirements", []).append(
                {
                    "code": str(item.get("code") or "display_projection_review"),
                    "message": str(item.get("message") or ""),
                    "source_refs": [str(ref) for ref in item.get("source_refs") or []],
                }
            )
...
def apply_inline_marks(display: dict[str, Any], plan: dict[str, Any]) -> list[str]:
    actions: list[str] = []
    for mark in plan.get("inline_marks") or []:
        if not isinstance(mark, dict):
            continue
        area = str(mark.get("display_area") or "")
        text = str(mark.get("text") or "").strip()
        mark_type = str(mark.get("mark") or "")
        if area not in DISPLAY_TARGET_FIELDS or mark_type != "underline" or not text:
            continue
        current = str(display.get(area) or "")
        if f"<u>{text}</u>" in current or text not in current:
            continue
        display[area] = current.replace(text, f"<u>{text}</u>", 1)
        actions.append(f"program_renderer:inline_underline:{area}:{text[:40]}")
    return actions


def ensure_context_visible(packet: dict[str, Any], display: dict[str, Any]) -> list[str]:
    """Do not let non-empty support/context fields silently disappear in rendering."""
    question = packet.get("standard_question") or {}
    context = str(question.get("context") or "").strip()
    if not context:
        return []
    family = str(packet.get("packet_family") or "").lower()
    if family == "cloze" and packet.get("refine_status") != "PRESERVED_NON_DIRECT":
        return []
    display_text = "\n".join(str(display.get(field) or "") for field in DISPLAY_TARGET_FIELDS)
    if compact_for_length_compare(context) and compact_for_length_compare(context) in compact_for_length_compare(display_text):
        return []
    heading = "### 参考词汇" if family == "reading" else "### 上下文/补充"
    target = "stem_markdown" if family in {"reading", "writing"} else "translation_markdown"
    existing = str(display.get(target) or "").strip()
    display[target] = (existing + "\n\n" + heading + "\n" + context).strip() if existing else heading + "\n" + context
    blocks = display.get("rendering_blocks") if isinstance(display.get("rendering_blocks"), list) else []
    if "support_context" not in blocks:
        blocks.append("support_context")
        display["rendering_blocks"] = blocks
    return [f"program_renderer:context_field_preserved_in_{target}"]


def normalize_display_markdown_fields(display: dict[str, Any]) -> list[str]:
    actions: list[str] = []
    for field in DISPLAY_TARGET_FIELDS:
        original = str(display.get(field) or "")
        normalized = html.unescape(original).replace("\xa0", " ")
        normalized = normalize_loose_pipe_tables(normalized)
        if normalized != original:
            display[field] = normalized
            actions.append(f"program_renderer:display_markdown_normalized:{field}")
    return actions
...
def is_direct_cloze_packet(packet: dict[str, Any], source_fields: dict[str, str]) -> bool:
    if str(packet.get("packet_family") or "").strip().lower() != "cloze":
        return False
    if packet.get("refine_status") == "PRESERVED_NON_DIRECT":
        return False
    return bool(source_fields.get("passage") or source_fields.get("stem") or source_fields.get("options"))


def cloze_layout_section(
    section_id: str,
    display_area: str,
...
def enforce_cloze_layout_contract(plan: dict[str, Any], packet: dict[str, Any], source_fields: dict[str, str]) -> list[str]:
    """Cloze display is deterministic: shared stimulus may prepend, but never replaces local passage/stem/options."""
    if not is_direct_cloze_packet(packet, source_fields):
        return []
    sections: list[dict[str, Any]] = []
    if source_fields.get("examples"):
        sections.append(cloze_layout_section("source_label", "stem_markdown", ["examples"], "cloze source label/examples precede the item"))
    if source_fields.get("resolved_stimulus"):
        sections.append(cloze_layout_section("shared_stimulus", "stem_markdown", ["resolved_stimulus"], "shared cloze stimulus is displayed before the local item"))
    if source_fields.get("passage"):
        sections.append(cloze_layout_section("local_passage", "stem_markdown", ["passage"], "local cloze passage is required and must not be replaced by shared stimulus"))
    if source_fields.get("stem"):
        sections.append(cloze_layout_section("local_stem", "stem_markdown", ["stem"], "local cloze stem/instruction is required when present"))
    if source_fields.get("options"):
        sections.append(cloze_layout_section("options", "stem_markdown", ["options"], "cloze options follow the local item"))
    if source_fields.get("answer"):
        sections.append(cloze_layout_section("answer", "answer_markdown", ["answer"], "answer field maps to answer display"))
    if source_fields.get("analysis"):
        sections.append(cloze_layout_section("analysis", "analysis_markdown", ["analysis"], "analysis field maps to analysis display"))
    if source_fields.get("translation"):
        sections.append(cloze_layout_section("translation", "translation_markdown", ["translation"], "option glosses and translation stay after analysis"))
    if sections:
        plan["layout_sections"] = sections
    bindings = plan.setdefault("binding_decisions", {})
    stimulus = bindings.get("stimulus") if isinstance(bindings.get("stimulus"), dict) else {}
    if source_fields.get("resolved_stimulus"):
        ...
        stimulus.update(
            {
                "required": True,
                "status": "BOUND",
                "reason": "cloze shared stimulus is display support and must not replace local passage/stem/options",
            }
        )
        bindings["stimulus"] = stimulus
    return ["program_renderer:cloze_layout_contract"]


def execute_render_plan(packet: dict[str, Any], plan: dict[str, Any]) -> dict[str, Any]:
    source_fields = standard_source_fields(packet)
    contract_actions = enforce_cloze_layout_contract(plan, packet, source_fields)
    ...
    display = {
        ...
        "items": [],
        "rendering_blocks": [],
        "surface_assets": surface_assets_for_packet(packet),
    }
    ...
    actions.extend(apply_inline_marks(display, plan))
    actions.extend(ensure_context_visible(packet, display))
    actions.extend(normalize_display_markdown_fields(display))
    ...
    if display["surface_assets"]:
        for asset in display["surface_assets"]:
            kind = str(asset.get("kind") or "")
            if kind == "missing_surface_asset":
                continue
            if (packet.get("asset_refs") or {}).get("writing_surface_refs"):
                display["rendering_blocks"].append("writing_surface")
            else:
                display["rendering_blocks"].append("visual_surface")
...
def validate_record(record: dict[str, Any], packet: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(packet.get("packet_family") or "").strip().lower() == "cloze" and record.get("render_status") != "BLOCKED":
        stem_compact = compact_for_length_compare(strip_inline_html(stem_markdown))
        for source_key in ["passage", "stem"]:
            source_text = str(standard_question.get(source_key) or "").strip()
            source_compact = compact_for_length_compare(source_text)
            if len(source_compact) < 12:
                continue
            if source_compact not in stem_compact:
                errors.append(
                    {
                        "path": "$.display_question.stem_markdown",
                        "message": f"cloze local {source_key} is missing or replaced in display; shared stimulus must not overwrite local item content",
                    }
                )
    ...
    if source_blank_runs and display_blank_runs < source_blank_runs:
        ...
        if renderable_surface_asset_present(display):
            warnings.append(
                {
                    **issue,
                    "message": issue["message"] + "; renderable source surface is attached, so this is a visual-fidelity warning instead of text-render failure",
                }
            )
        else:
            errors.append(issue)
...
def strip_inline_html(text: str) -> str:
    return re.sub(r"</?u>", "", str(text or ""))


def blank_run_html(length: int) -> str:
    width = max(72, min(420, length * 9))
    return f"<span class='fill-blank' style='min-width:{width}px'></span>"


def componentize_blank_runs(line: str) -> str:
    out: list[str] = []
    run = 0
    run_char = ""
    ...
    def flush_run() -> None:
        ...
        if not run:
            return
        out.append(blank_run_html(run) if run >= 3 else html.escape(run_char * run))
        run = 0
        run_char = ""

    for char in str(line or "").replace("&nbsp;", " ").replace("\xa0", " "):
...
def componentize_markdown(markdown: str) -> str:
    lines: list[str] = []
    for raw_line in str(markdown or "").replace("&nbsp;", " ").replace("\xa0", " ").splitlines():
        stripped = raw_line.strip()
        if stripped and set(stripped) <= SURFACE_BLANK_CHARS and len(stripped) >= 3:
            lines.append("<div class='write-line'></div>")
            continue
        lines.append(componentize_blank_runs(raw_line))
    return "\n".join(lines)
...
def markdown_to_html(markdown: str) -> str:
    ...
    if not text:
        return "<div class='empty'>（空）</div>"
    text = normalize_loose_pipe_tables(text)
    return MARKDOWN_RENDERER.render(componentize_markdown(text)) or "<div class='empty'>（空）</div>"


def render_surface_assets(display: dict[str, Any]) -> str:
    assets = display.get("surface_assets") if isinstance(display.get("surface_assets"), list) else []
    if not assets:
        return ""
    cards: list[str] = []
    ...
    for asset in assets:
        ...
        if not path_text:
            continue
        abs_path = workspace_path(path_text)
        if not abs_path.exists():
            missing.append(
                {
                    "kind": "missing_surface_asset",
                    "path": path_text,
                    "caption": "surface asset path does not exist",
                }
            )
            continue
        ...
        cards.append(
            ...
            f"<a href='{html.escape(url)}' target='_blank'><img src='{html.escape(url)}'></a>"
            f"<figcaption>{html.escape(str(asset.get('caption') or Path(path_text).name))}</figcaption>"
            "</figure>"
        )
    missing_html = ""
    if missing:
        missing_html = (
            "<details class='surface-missing'><summary>缺少可渲染视觉资产</summary>"
            f"<pre>{html.escape(json.dumps(missing, ensure_ascii=False, indent=2))}</pre></details>"
        )
    if not cards and not missing_html:
        return ""
    return (
        "<h3>视觉/作答区</h3>"
        "<div class='surface-panel'>"
        f"<div class='surface-grid'>{''.join(cards)}</div>"
        f"{missing_html}"
        "</div>"
    )
...
.surface-panel{{background:#fbfdff;border:1px solid #d8dde6;border-radius:6px;padding:10px;margin:0 0 12px}} .surface-grid{{display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start}} .surface-card{{max-width:520px}} .surface-card img{{width:500px;max-width:100%;border:1px solid #b8c2d1;background:white}} .surface-missing{{margin-top:8px}}
...
.fill-blank{{display:inline-block;border-bottom:1px solid #111827;height:.95em;vertical-align:-0.12em;margin:0 4px}} .rendered u{{text-underline-offset:3px;text-decoration-thickness:1px}}
...
.md-table,.rendered table{{border-collapse:collapse;width:100%;margin:8px 0 12px;font-size:14px}} .md-table th,.md-table td,.rendered th,.rendered td{{border:1px solid #cfd6df;padding:7px 9px;vertical-align:top}} .md-table th,.rendered th{{background:#f3f6fa;text-align:left}}
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    display_projection_by_group: dict[str, dict[str, Any]] = {}
    ...
    if args.display_projection_plan_json:
        display_projection_payload = read_json(workspace_path(args.display_projection_plan_json))
        for projection in display_projection_payload.get("display_projections") or []:
            group_id = projection.get("source_group_id")
            if group_id:
                display_projection_by_group[group_id] = projection
        packets = [
            with_display_projection(packet, display_projection_by_group.get(packet.get("source_group_id")))
            for packet in packets
        ]
    ...
    def process_packet(packet: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            plan = apply_display_projection_floor(repair_plan_shape(model_result["parsed"], packet), packet)
            ...
            if not plan_validation["valid"]:
                ...
                retry_plan = apply_display_projection_floor(repair_plan_shape(retry_result["parsed"], packet), packet)
            ...
            if not plan_validation["valid"]:
                plan = apply_display_projection_floor(
                    repair_plan_shape(None, packet, "Model render instruction plan failed schema validation; program renderer used baseline source-field copy."),
                    packet,
                )
                plan_validation = validate_render_plan(plan)
            ...
            plan = apply_display_projection_floor(repair_plan_shape(None, packet, f"model_call_failed: {type(exc).__name__}: {exc}"), packet)
            plan_validation = validate_render_plan(plan)
            record = execute_render_plan(packet, plan)
            record["render_status"] = "NEEDS_REVIEW"
            record.setdefault("unresolved_issues", []).append(
                {"code": "planner_model_call_failed", "message": f"{type(exc).__name__}: {exc}", "source_refs": []}
            )
            record.setdefault("normalization_actions", []).append("program_renderer:fallback_after_model_call_failed")
            postprocess_rendered_record(record, packet)
            finalize_record_posture(packet, record)
...
def main() -> None:
    ...
    parser.add_argument("--display-projection-plan-json", default="")
