...
class DeliveryController {
    ...
    private final DeliveryService service; private final DeliveryContract contract;
    DeliveryController(DeliveryService service,DeliveryContract contract) { this.service=service; this.contract=contract; }
    ...
    JsonNode preflight(@RequestHeader("X-Actor-Id") UUID actor,HttpServletRequest request) throws java.io.IOException { return service.preflight(manifest(request),actor); }
    @PostMapping("/imports")
    ResponseEntity<JsonNode> commit(@RequestHeader("X-Actor-Id") UUID actor,HttpServletRequest request) throws java.io.IOException {
        var result=service.commit(manifest(request),actor); return ResponseEntity.status(result.replay()?200:201).body(result.receipt());
    }
    @GetMapping("/requests/{requestId}")
    JsonNode receipt(@PathVariable UUID requestId,@RequestParam UUID workspaceId,@RequestHeader("X-Actor-Id") UUID actor) { return service.receipt(workspaceId,requestId,actor); }
