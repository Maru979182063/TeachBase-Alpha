...
- 文档名称：文档 Profile Resolver 与语义 Role Adapter 设计方案
- 适用分支：`validation/backend-runtime-20260706`
- 输出文件：`docs/semantic_profile_role_adapter_prd_v0.1.md`
- 文档状态：PRD / 设计与验收方案
- 本轮范围：只产出 PRD、数据契约、接入方案、测试方案和实施拆分，不修改运行时代码。
- 核心原则：视觉结构为主，文本/OCR/PDF text 只作为辅助证据；新增语义适配层不得绕过 split_v03、transcription、asset audit、release decision、lineage 和 Runtime 既有门闸。
...
- v02 的 `unit_kind` 可作为 role evidence，但不能直接替代 canonical role。
...
- `Semantic Role Adapter`：以 split_v03 已组装的 semantic node 为主输入，输出 canonical role、profile subtype、route、confidence、evidence、review/fallback。
...
- 保持 canonical role 稳定、克制，学科差异通过 subtype 和配置扩展。
...
flowchart TD
  ...
  F -->|"passage_or_material"| J["Shared Material Preserve"]
...
```json
{
  "profile_version": "document_profile_v0.1",
  "document_id": "",
  "source_run_id": "",
...
- 必须字段：`profile_version`、`subject`、`document_type`、`content_mode`、`stage`、`language`、`confidence`、`source`、`needs_profile_review`。
- 可空字段：`document_id`、`source_run_id`、`prompt_version`、`config_version`，但进入 lineage 时应补齐。
- `content_mode` 可以多选，例如英语阅读讲义可同时是 `reading`、`worked_examples`、`exercise_driven`。
...
```json
{
  "adapter_version": "semantic_role_adapter_v0.1",
  "source_run_id": "",
  "document_profile_id": "",
  "node_id": "",
  "node_type": "",
  "canonical_role": "knowledge|worked_example|exercise|question_group|passage_or_material|answer_explanation|table_or_diagram|method_or_strategy|mixed|noise|review_only",
  "profile_subtype": "",
...
### 10.3 Canonical role 枚举

| canonical_role | 定义 | 是否可直接进题库 | 默认 route |
|---|---|---:|---|
...
| `passage_or_material` | 阅读文章、实验材料、公共背景材料 | 否，作为 shared context | `shared_material_preserve` |
| `answer_explanation` | 答案、解析、翻译、详解、参考解 | 否，需挂载 | `answer_attachment` |
| `table_or_diagram` | 表格、树状图、知识图谱、实验图示、函数/几何图组 | 否，作为资产或知识内容 | `visual_asset_preserve` |
| `method_or_strategy` | 方法总结、技巧点拨、解题策略、模板步骤 | 否 | `knowledge_transcription` |
| `mixed` | 同一 node 内含多种功能，不能直接下游 | 否 | `secondary_visual_split` |
| `noise` | 页眉、页脚、logo、页码、装饰 | 否 | `noise` |
| `review_only` | 证据冲突或低置信，不应自动路由 | 否 | `review_only` |
...
| role | 默认 route | profile 可覆盖 | 进入 question bank |
|---|---|---:|---:|
...
- 不把 `unit_kind` 直接作为 canonical role。
...
- adapter 只输出 `canonical_role / route / needs_role_review / confidence`。
...
```json
{
  "prompt_version": "semantic_role_adapter_v0.1",
  "config_version": "semantic_profiles_v0.1",
...
question ingest 只消费已被 route 到题目链路的节点。对于 `knowledge`、`method_or_strategy`、`passage_or_material`，不得伪装为 question source。
