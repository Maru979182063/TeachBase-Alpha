...
- ` M` `mixed_control_file` `package.json`: foundation_hardening npm script; final_chain_registry npm scripts; precleanup_archive npm scripts; precleanup_deep_audit npm script; precleanup_safety npm script; cleanroom_goal_gap audit npm script
...
- `??` `final_chain_registry` `docs/reports/cleanroom_goal_gap_audit_20260804.json`: protected-chain inventory, classifier, or cleanup candidate audit
- `??` `final_chain_registry` `docs/reports/cleanroom_goal_gap_audit_20260804.md`: protected-chain inventory, classifier, or cleanup candidate audit
- `??` `final_chain_registry` `tests/test_cleanroom_goal_gap_audit.py`: protected-chain inventory, classifier, or cleanup candidate audit
- `??` `final_chain_registry` `tools/build_cleanroom_goal_gap_audit.py`: protected-chain inventory, classifier, or cleanup candidate audit
